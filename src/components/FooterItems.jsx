  ...
  return (
    <div className='my-8 md:my-0'>
            <p className='text-white font-semibold text-[16px] flex justify-between items-center' onClick={handleMouseClick}>
                ...
                <span className={`md:hidden hover:cursor-pointer transition-all duration-500 ${isOpen ? 'rotate-180' : 'rotate-0'}`}>
            ...
            <ul className={`${isOpen ? 'block' : 'hidden'} md:block`}>
                {props.footerDropdowns.items.map((items, index) => (
                    <li className='md:my-6 text-white font-normal text-[16px] ml-4 md:ml-0' key={index}>{items}</li>
                ))}
            </ul>
    </div>
  )
